...
import { getArticleColumns, createArticleColumn, updateArticleColumn, deleteArticleColumn } from '@/api/article'
...
async function fetchList() {
  loading.value = true
  try {
    const res = await getArticleColumns({ keyword: keyword.value || undefined })
...
/** location 合法性校验（小写字母/数字/下划线/连字符） */
const locationReg = /^[a-z0-9_-]+$/

function openCreate() {
  editingId.value = null
  modalTitle.value = '新建栏目'
...
async function handleOk() {
  if (!form.name.trim()) return message.warning('请输入栏目名称')
  ...
  try {
    if (editingId.value) {
      await updateArticleColumn(editingId.value, { ...form })
      message.success('修改成功')
    } else {
      await createArticleColumn({ ...form })
...
function handleDelete(record) {
  Modal.confirm({
    title: `确定删除栏目「${record.name}」？`,
    content: '若该栏目下存在文章，将无法删除，请先移除文章。',
    okType: 'danger',
    onOk: async () => {
      await deleteArticleColumn(record.id)
      message.success('已删除')
      fetchList()
    },
  })
...
<template>
  <div>
    <div class="action-bar">
      ...
      <a-button type="primary" @click="openCreate">+ 新建栏目</a-button>
    </div>
    ...
    >
      <a-form layout="vertical" style="margin-top:8px">
        ...
        <a-form-item required>
          <template #label>
            location 标识
            <span style="font-size:12px;color:#aaa;font-weight:400;margin-left:6px">C 端通过此标识查询栏目文章列表</span>
          </template>
          <a-input
            v-model:value="form.location"
            placeholder="小写字母/数字/下划线/连字符，如 about-us"
            :maxlength="64"
          />
        </a-form-item>
